...
def cvt_pcd_to_bin(pcd_dir):
    for (root, dirs, files) in os.walk(pcd_dir):
        files.sort()
        for filename in files:
            if ('.pcd' not in filename or 'labeling_data' not in root) or '.bin' in filename:
                continue
            pcd_data = o3d.io.read_point_cloud(root + '/' + filename)
            npy_data = np.asarray(pcd_data.points)
            bin_tank = np.zeros((npy_data.shape[0], 5), dtype=np.float32)
            bin_tank[:, :3] = npy_data
            # rot 90
            bin_tank = bin_tank[:, [1, 0, 2, 3, 4]]
            #  bin_tank[:, 1] = -bin_tank[:, 1]
            ########
            bin_tank.tofile(root + '/' + filename + '.bin')
            if False:
                points = np.fromfile(root + '/' + filename + '.bin', dtype=np.float32,
                                     count=-1).reshape([-1, 5])[:, :4]
                bev_map = nuscene_vis(points)
                cv2.imwrite('rotate_test.png', bev_map)
                breakpoint()
                abcd = 1
